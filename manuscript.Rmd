...
The aim was to determine the accuracy of machine learning models for predicting if a period of apnea will persist for 30 seconds or more. 
...
Net benefit associated with the random forest model exceeded the strategy of intervening for all patients across all probability thresholds from the range of 0.4 to 0.8. The interpretation is that the best clinical outcome would be achieved for clinicians who are willing to initiate an intervention to treat apnea at the 15 second mark if the probability of it being prolonged was more than 40%, would be achieved by using the random forest model as opposed to treating everyone. In contrast, the net benefit associated with the random forest model was lower than the strategy of not intervening for any patients across all probability thresholds from the range of 0.4 to 0.8. This was due to the fact that we applied a condition that we would not accept more than 5 false positives to detect 1 true positive. The number of false positives that triggered across probability thresholds from 0.4 to 0.8 using the random forest model would far exceed the number of true positives, meaning the preferred strategy should remain as only setting the alert for the 30-second time-point.
